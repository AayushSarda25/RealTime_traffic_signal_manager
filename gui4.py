...
class TrafficSimulatorGUI:
    ...
    def create_lane_box(self, parent, lane, row, col):
        ...
        btn_upload = ttk.Button(frame, text="Upload Image", command=lambda l=lane: self.upload_image(l))
        btn_upload.pack()

        btn_camera = ttk.Button(frame, text="Capture from Camera", command=lambda l=lane: self.capture_from_camera(l))
        btn_camera.pack()
    ...
    def capture_from_camera(self, lane):
        cap = cv2.VideoCapture(0)

        if not cap.isOpened():
            self.output_text.insert(tk.END, "Failed to open camera.\n")
            return

        start_time = time.time()
        frame_list = []

        self.output_text.insert(tk.END, f"Capturing 3 seconds video for {lane} lane...\n")
        self.root.update()
        ...
        vehicle_count = 0
        for frame in frame_list[::5]:  # Sample every 5th frame
            results = model(frame)
            for r in results:
                for box in r.boxes:
                    if int(box.cls) in vehicle_classes:
        ...
        vehicle_count = vehicle_count // max(len(frame_list)//5, 1)

        self.vehicle_entries[lane].delete(0, tk.END)
        self.vehicle_entries[lane].insert(0, str(vehicle_count))
        self.output_text.insert(tk.END, f"{lane} Lane -> Vehicles detected from camera: {vehicle_count}\n")
